...
int make_matrices(PROT prot, char *dir)
{
    ...
    char confName[6];
    int natom;
    // int serial; // Commented out by Salah on Dec. 2015 (not used)
    char uniqID[15];
    char dummy;
    EMATRIX ematrix;
	

	//if (verbose != 1) { // Commented out by Salah on Dec. 2015 (not used)
	verbose = 0; 
	//}
...
// You need this function here (extract_matrix) becuase the zopp program uses it. Don't think its not used :) 
