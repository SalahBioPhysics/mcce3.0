...
// This the pairwise interactions 
int conf_energies(int kr, int kc, PROT prot)
{
   ...
   if (!env.skip_ele) {
	  ...
	  /*conformer is not charged; skipping PBE calculations*/
		  ...
		  if (!strcmp(env.pbe_solver, "apbs")) { // If you are using apbs option in run.prm.  (Salah Nov. 2015)
		  ...
		  else { // If you are using delphi option in run.prm.  (Salah Nov. 2015)
			// RUN DELPHI CALCULATIONS
			// Added by Salah, old delphi vs. new delphi"v68" (11/2015)
			if (!strcmp(env.delphi_exe_home, "delphi1")){
				...
				//===== Added by Salah (10/2015) , needed for the new DelPhi (v68) ===== 
				fp2 = fopen("apbs.pqr", "w");
				if (write_pqr(fp2)) {
				   printf("   FATAL: Cannot write a pqr file in conf_energies()\n");
				   fclose(fp2);
			...
			//if (env.display_delphi_io_intial > 0){
				   //fp_delphiIO = fopen("delphiInputs.txt","a+");
				   //fprintf(fp,"%d %d\n",env.display_delphi_io_intial, env.display_delphi_io_final);
				   //if (kc >= env.display_delphi_io_intial && kc <= env.display_delphi_io_final){
					   /*for (i=0; i<ele_bound.n; i++){
								   fprintf(fp_delphiIO,"%s%10.3f%8.3f%8.3f%8.3f%8.3f\n", 
								   ele_bound.head[i],
								   ele_bound.unf[i].x,
								   ele_bound.unf[i].y,
								   ele_bound.unf[i].z,
			...
			while (notpassed){
			  ...
			  printf("ATOM  %5d  X   XXX  %04d    %8.3f%8.3f%8.3f%8.3f%8.3f\n", i, i,
			   ...
			   system("cat fort.27");
			   */
			   // fort.10 is the input file for delphi. 	
			   //==== Keep all inputs to delphi ==========
			   ...
			   fp = fopen("fort.10", "w");
			   if (!strcmp(env.delphi_exe_home, "delphi1")){
				   // The following are the input parameters needed by the old delphi (Salah 2015)
				   ...
				   // The following are input parameters needed by the new delphi v68 (Salah 2015)
				   ...
				   if (env.do_gaussian_smooth == 1){
					   fprintf(fp, "gaussian=%d\n",env.do_gaussian_smooth);
					   fprintf(fp, "sigma=%.2f\n",env.gaussian_sigma);
					   fprintf(fp, "srfcut=%.2f\n",env.gaussian_srfcut);
				   }
			   }
			   fprintf(fp, "gsize=%d\n", env.grids_delphi);
			   ...
			   if (n_retry<2) {
				   // Run delphi as a system call. 
				   system(sbuff);
			   }
			   else if (n_retry>=2 && n_retry<100) {
				   // Moving the center a little, that's if delphi didn't work
				   // that's because sometimes the center sit on a grid. 
				  if(!strcmp(env.delphi_exe_home, "delphi1")){
					  ...
					  fclose(fp);
				  }else{ // Moving the center for new DelPhi"v68" by Salah (11/2015)
					  center.x = center.x + 0.1*((float) rand() / RAND_MAX - 0.5);
					  center.y = center.y + 0.1*((float) rand() / RAND_MAX - 0.5);
					  center.z = center.z + 0.1*((float) rand() / RAND_MAX - 0.5);
				  }
				  printf("   Trying changing scale on trial %d\n", n_retry);
				  system(sbuff);
			   }
			   else {
				  printf("   FATAL: too many failed delphi runs (%d). \n\tCheck the path to the DelPhi executable file, quitting...\n", n_retry);
				  return USERERR;
			   }
			   ...
			   for (i=1; i<del_runs; i++) {
				  ...
				  if (!strcmp(env.delphi_exe_home, "delphi1")){
					  ...
					  fprintf(fp, "energy(g,an,ag,sol)\n");
				  }else{ // Added by Salah (10/2015) this is needed for the new DelPhi (v68)
					  fprintf(fp, "in(modpdb4,file=\"apbs.pqr\",format=pqr)\n"); 
			   ...
			   if (!(fp = fopen(fname, "r"))){
			    ...
			    // skip 12 lines, that becuase the first 12 lines in run01.frc are not used. (Salah Nov. 2015) 
			...
			for (i=0; i<del_runs; i++) {
			   ...
			   while (fgets(sbuff, sizeof(sbuff), fp2)) {
				  if (strstr(sbuff, "corrected reaction field energy:") || strstr(sbuff, "corrected reaction field energy")) {
					 if (strstr(sbuff, "NaN")) rxn_corrected = 999.99;
					 else if(!strcmp(env.delphi_exe_home, "delphi1")){
							rxn_corrected = atof(sbuff+34)/KCAL2KT;
							//fprintf(fp_delphiIO,"corrected reaction field energy:%10.3f\n",rxn_corrected);
						 }else{ // For new delphi by Salah (11/2015)
							rxn_corrected = atof(sbuff+39)/KCAL2KT; // You can find this value (rxn_corrected) in delphi0#.log in PBE_FOLDER if you set  (DELPHI_CLEAN) flag to f
							//fprintf(fp_delphiIO,"corrected reaction field energy salah:%10.3f\n",rxn_corrected);
						 }
					 break;
				  }
			   }
...
{  int kr, kc, ka, i, counter;
   ...
   for (kr=0; kr<prot.n_res; kr++) {
      ...
      for (ka=0; ka<prot.res[kr].conf[0].n_atom; ka++) {
         ...
         sprintf(ele_bound.head[counter], "ATOM  %5d %4s%c%3s %c%04d%c%03d",
                            counter%100000, 
							prot.res[kr].conf[0].atom[ka].name,
...
// any conf with all first confs of all residues
int conf_rxn(int kr, int kc, PROT prot)
{  
      ...
      if (weight < 0.0001) {
		 ...
		 if (!strcmp(env.pbe_solver, "apbs")) {
			...
			*/
			/*
			if (env.display_delphi_io_intial > 0){
				   fp = fopen("delphiInputs2.txt","a+");
				   //fprintf(fp,"%d %d\n",env.display_delphi_io_intial, env.display_delphi_io_final);
				   if (kc >= env.display_delphi_io_intial && kc <= env.display_delphi_io_final){
					   for (i=0; i<prot.res[kr].conf[kc].n_atom; i++){
						   fprintf(fp, "ATOM  %5d %4s %3s %c%04d    %8.3f%8.3f%8.3f %6.3f %4.2f\n",
										i, prot.res[kr].conf[kc].atom[i].name,
										prot.res[kr].resName,
										prot.res[kr].chainID,
										prot.res[kr].resSeq,
										prot.res[kr].conf[kc].atom[i].xyz.x,
										prot.res[kr].conf[kc].atom[i].xyz.y,
										prot.res[kr].conf[kc].atom[i].xyz.z,
										prot.res[kr].conf[kc].atom[i].crg,
										prot.res[kr].conf[kc].atom[i].rad);
					   }
				   }
				   fclose(fp);
			   }*/
			...
			fprintf(fp, "out(eps,file=\"epsmap.txt\")\n");
			fclose(fp);
			sprintf(sbuff, "%s>rxn01.log 2>/dev/null", env.delphi_exe);
			system(sbuff);
			...
			for (i=1; i<del_runs; i++) {
			   ...
			   fprintf(fp, "out(eps,file=\"epsmap.txt\")\n");
			   fclose(fp);
			   sprintf(sbuff, "%s>rxn%02d.log 2>/dev/null", env.delphi_exe, i+1);
			   system(sbuff);
			}
					   ...
					   while (fgets(sbuff, sizeof(sbuff), fp2)) {
						  if (strstr(sbuff, "corrected reaction field energy:") || strstr(sbuff, "corrected reaction field energy")) {
							 if (strstr(sbuff, "NaN")) rxn[i] = 999.99;
									...
									rxn[i] = atof(sbuff+39)/KCAL2KT; // Changed from 34 to 39 (by Salah)
								 }
							 break;
						  }
			...
			if (!strcmp(env.rxn_method,"ntsurface")) {
				...
				/*OTHERWISE RUN DELPHI AGAIN TO CALCULATE RXN AS PER THE TPL FILES*/
					...
					if(!strcmp(env.delphi_exe_home, "delphi1")){
						...
						fp2 = fopen("apbs.pqr", "w");
						if (write_pqr(fp2)) {
						   printf("   FATAL: Cannot write a apbs file in conf_energies()\n");
						   fclose(fp2);
						   return USERERR;
						}
					}
