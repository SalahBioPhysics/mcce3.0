...
int rotamers_GA(int argc, char *argv[]) {
   ...
   PROT prot;
   CONFSTAT confstat;
   int c, i, j, kr, kc,ic,k;
   // char sbuff[MAXCHAR_LINE]; // Commented out by Salah on Dec. 2015 (not used)

   nowStart = time(NULL);
   idum = time(NULL);

   FILE *pdb_fp;
   // int i_res, i_conf, i_atom; // Commented out by Salah on Dec. 2015 (not used)
   // char line[MAXCHAR_LINE]; // Commented out by Salah on Dec. 2015 (not used)
...
/* Spherical Focus Code */
   //int ga_focus_resid; // Commented out by Salah on Dec. 2015 (not used)
   //float ga_focus_probe_radius; // Commented out by Salah on Dec. 2015 (not used)
