...
void GA_SETUP(PROT *prot, GA_STRUCTURE gs, int patch_nb) {
   ...
   //struct timeb start,end; // Commented out by Salah on Dec. 2015 (not used)
   ...
   if((4*s_total)/(float)1048576 < phy_av*psize/(float)1048576) {
	fprintf(stdout,"GA: Pre-calculating pairwise conformer VDW interactions please wait... "); fflush(stdout);
   	double sum = 0.0f;
   	int i_idx, j_idx; //, tmp_idx; // Commented out by Salah on Dec. 2015 (not used)
...
void evaluate_population2(PROT prot, CHROMOSOME *pop) {
	int i,j,k,m,i_idx,j_idx,p; // tmp_idx // Commented out by Salah on Dec. 2015 (not used)
	//#pragma omp parallel for private(i, j, k, m, i_idx, j_idx, tmp_idx) // Commented out by Salah on Dec. 2015 (not used)
...
void RUN_GA(PROT *prot, int patch_nb, int eval_p) {
	...
	int **res;
        res = calloc(prot->n_res,sizeof(int*));
        //int count=0; // Commented out by Salah on Dec. 2015 (not used)
