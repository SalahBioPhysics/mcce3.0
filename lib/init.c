...
{   FILE *fp;
	...
	// All of the default vlaues used by mcce, one can change these values in run.prm (comment by Salah 11/2015)
    // Default values 
    env.test_seed = -1;
    env.minimize_size = 0;
    env.PI                = 4.*atan(1.);
    ...
    env.prune_ele         = 2.0;
    env.prune_vdw         = 2.0;
	env.display_delphi_io_intial = 0;
	env.display_delphi_io_final = 0;
	...
	env.do_gaussian_smooth = 0;
	env.gaussian_sigma    = 0.95;
	env.gaussian_srfcut   = 20.0;
    env.monte_adv_opt     =    0;
    ...
    // This while loop reads all the values from run.prm (comment by Salah 11/2015)
    // user values 
    while (fgets(sbuff, sizeof(sbuff), fp)) {
    	...
    	if (strstr(sbuff, "(DO_TRACE)")) {
			...
			if (strstr(sbuff, "delphi1") || strstr(sbuff, "Delphi1")) {
                strcpy(env.delphi_exe_home, "delphi1");
				sprintf(delphiexe_home, "%s", strtok(sbuff, " "));
			} // delphi2 is the new delphi, made in Nov. 2015, by Salah  
            if(strstr(sbuff, "delphi2") || strstr(sbuff, "Delphi2")){
				strcpy(env.delphi_exe_home, "delphi2"); 
				sprintf(delphiexe_home, "%s", strtok(sbuff, " "));
			} 
			strcpy(env.delphi_exe, delphiexe_home);
        }
		else if (strstr(sbuff, "(GAUSSIAN_SMOOTH)")) {
            str1 = strtok(sbuff, " ");
            if (str1[0] == 't' || str1[0] == 'T') {
                env.do_gaussian_smooth = 1;
            }
            else env.do_gaussian_smooth = 0;
        }
		else if (strstr(sbuff, "(GAUSSIAN_SIGMA)")) {
            env.gaussian_sigma = atof(strtok(sbuff, " "));
        }
        ...
        else if (strstr(sbuff, "(PBE_START)") || strstr(sbuff, "(DELPHI_START)")) {
            env.pbe_start = atoi(strtok(sbuff, " "));
        }
        else if (strstr(sbuff, "(PBE_END)") || strstr(sbuff, "(DELPHI_END)")) {
            env.pbe_end = atoi(strtok(sbuff, " "));
        }
		else if (strstr(sbuff, "(DELPHI_IO_INTIAL)")) {
            env.display_delphi_io_intial = atoi(strtok(sbuff, " "));
        }
		else if (strstr(sbuff, "(DELPHI_IO_FINAL)")) {
            env.display_delphi_io_final = atoi(strtok(sbuff, " "));
        }
	...
	// This if statment is only to load all of the used values from run.prm to run.trace (comment by Salah 11/2015)
	if (dotrace == 1) {
        ...
        while (fgets(trbuff, sizeof(trbuff), fp)) {
			if (strstr(trbuff, "(ADDING_CONF)")) {
				...
				fprintf(tr, "%31s%31s\t%31s%31d\n", "(IPECE_ADD_MEM)", strtok(trbuff, " "), "env.ipece.add_mem", env.ipece.add_mem);
			}
			else if (strstr(trbuff, "(IPECE_GRID_SPACE)")) {
				fprintf(tr, "%31s%31s\t%31s%31f\n", "(IPECE_GRID_SPACE)", strtok(trbuff, " "), "env.ipece.grid_space", env.ipece.grid_space);
			}
			else if (strstr(trbuff, "(IPECE_MEM_CHAINID)")) {
				fprintf(tr, "%31s%31s\t%31s%31d\n", "(IPECE_MEM_CHAINID)", strtok(trbuff, " "), "env.ipece.mem_chainID", env.ipece.mem_chainID);
			}
			else if (strstr(trbuff, "(IPECE_MEM_THICKNESS)")) {
				fprintf(tr, "%31s%31s\t%31s%31f\n", "(IPECE_MEM_THICKNESS)", strtok(trbuff, " "), "env.ipece.half_mem_thickness", env.ipece.half_mem_thickness);
			}
			else if (strstr(trbuff, "(IPECE_MEM_ATOM_RADIUS)")) {
				fprintf(tr, "%31s%31s\t%31s%31f\n", "(IPECE_MEM_ATOM_RADIUS)", strtok(trbuff, " "), "env.ipece.grid_space", env.ipece.grid_space);
			}
